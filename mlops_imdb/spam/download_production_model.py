"""Download the latest production-tagged spam eval pyfunc model from MLflow."""
...
def find_latest_production_eval_run(exp_id: str) -> Optional[object]:
    ...
    runs = client.search_runs(
        ...
        filter_string="tags.stage = 'spam_eval'",
    ...
    def is_production_tagged(run_tags: dict) -> bool:
        ...
        if "labels" in run_tags and "production" in str(run_tags["labels"]).lower():
            return True
        # Some backends store labels as tag keys like "label.production": "true"
...
def download_model(run_id: str, dst_dir: str) -> str:
    ...
    local_path = client.download_artifacts(run_id, "spam_model", dst_dir)
...
def main():
    parser = argparse.ArgumentParser(
        description="Download latest production-tagged spam eval model artifact from MLflow."
    )
    parser.add_argument(
        "--output-dir",
        default="models/spam_model_production",
        help="Local directory where the spam_model artifact folder will be downloaded.",
    )
    parser.add_argument(
        "--experiment-name",
        default=None,
    ...
    if not run:
        raise SystemExit(
            "No production-tagged spam_eval run found. "
            "Ensure the run has tag 'stage=spam_eval' and a 'production' label/tag."
        )
    ...
    if meta:
        ...
        # Only skip download when the upstream run start time matches the locally stored one.
        ...
        if prev_run_start_dt and latest_start == prev_run_start_dt:
            ...
            print(
                f"Local spam_model start time matches upstream. "
    ...
    print(
        f"Downloaded spam_model artifact from run {run.info.run_id} "
